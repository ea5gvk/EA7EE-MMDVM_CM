...
<?php if (!empty($_POST)):
	...
	if (empty($_POST['selectedAPRSHost']) != TRUE ) {
	  ...
	  $configysf2dmr['aprs.fi']['Enable'] = "1";
	...
	if (empty($_POST['confFREQtx']) != TRUE && empty($_POST['confFREQrx']) != TRUE ) {
	  ...
	  $configysf2dmr['YSF Network']['Suffix'] = "";
	...
	if (empty($_POST['ysf2dmrMasterHost']) != TRUE ) {
	  ...
	  if (substr($ysf2dmrMasterHostArr[3], 0, 2) == "BM") {  
		$configysf2dmr['DMR Network']['EnableUnlink'] = "1";
		}

	  if (substr($ysf2dmrMasterHostArr[3], 0, 4) == "DMR+") {
		$configysf2dmr['DMR Network']['EnableUnlink'] = "0";
		}		
	}
	...
	if (empty($_POST['ysf2dmrCallsign']) != TRUE ) {
			$configysf2dmr['aprs.fi']['AprsCallsign'] = escapeshellcmd($_POST['ysf2dmrCallsign']);
	}
	
	if (empty($_POST['ysf2dmrBeaconText']) != TRUE ) {
			$configysf2dmr['aprs.fi']['Beacon'] = escapeshellcmd($_POST['ysf2dmrBeaconText']);
	}

	if (empty($_POST['ysf2dmrBeaconIcon']) != TRUE ) {
			$configysf2dmr['aprs.fi']['Icon'] = escapeshellcmd($_POST['ysf2dmrBeaconIcon']);
	}

	if (empty($_POST['ysf2dmrBeaconTime']) != TRUE ) {
			$configysf2dmr['aprs.fi']['BeaconTime'] = escapeshellcmd($_POST['ysf2dmrBeaconTime']);
	}

	if (empty($_POST['ysf2dmrAprsApikey']) != TRUE ) {
			$configysf2dmr['aprs.fi']['APIKey'] = escapeshellcmd($_POST['ysf2dmrAprsApikey']);
	}

	if (empty($_POST['ysf2dmrVoiceTime']) != TRUE ) {
			$configysf2dmr['Storage']['BeaconTime'] = escapeshellcmd($_POST['ysf2dmrVoiceTime']);
	}	

	if (empty($_POST['confFollowMode']) != TRUE ) {
		if (escapeshellcmd($_POST['confFollowMode']) === "OFF") {
			$configysf2dmr['aprs.fi']['HotSpotFollow'] = "0";
	    } else {
			$configysf2dmr['aprs.fi']['HotSpotFollow'] = "1";
	    }		
	}
	...
	if (empty($_POST['MMDVMModeYSF2DMR']) != TRUE ) {
          ...
          if (escapeshellcmd($_POST['MMDVMModeYSF2DMR']) == 'ON' )  { 
			...
			$configmmdvm['System Fusion']['RemoteGateway'] = "0";			
			$configmmdvm['System Fusion Network']['Enable'] = "1";			
			$configmmdvm['System Fusion Network']['LocalPort'] = "42000";		
			$configmmdvm['System Fusion Network']['GatewayPort'] = "42013";	
			system('sudo mv /usr/local/bin/YSF2DMR.old /usr/local/bin/YSF2DMR > /dev/null 2>/dev/null &');		// YSFGateway
			system('sudo systemctl stop ysfgateway.service > /dev/null 2>/dev/null &');		// YSFGateway
			system('sudo systemctl stop ysfgateway.timer > /dev/null 2>/dev/null &');		// YSFGateway
			system('sudo systemctl stop ysfparrot.service > /dev/null 2>/dev/null &');		// YSFGateway
			system('sudo systemctl stop ysfparrot.timer > /dev/null 2>/dev/null &');		// YSFGateway
			system('sudo systemctl disable ysfgateway.service > /dev/null 2>/dev/null &');	// YSFGateway
			system('sudo systemctl disable ysfgateway.timer > /dev/null 2>/dev/null &');	// YSFGateway
			system('sudo systemctl disable ysfparrot.service > /dev/null 2>/dev/null &');	// YSFGateway
			system('sudo systemctl disable ysfparrot.timer > /dev/null 2>/dev/null &');		// YSFGateway
			system('sudo systemctl enable ysf2dmr.service > /dev/null 2>/dev/null &');	// YSFGateway
			system('sudo systemctl enable ysf2dmr.timer > /dev/null 2>/dev/null &');		// YSFGateway
			system('sudo systemctl start ysf2dmr.service > /dev/null 2>/dev/null &');	// YSFGateway
			system('sudo systemctl start ysf2dmr.timer > /dev/null 2>/dev/null &');		// YSFGateway
			system('sudo mv /usr/local/bin/YSFGateway /usr/local/bin/YSFGateway.old > /dev/null 2>/dev/null &');		// YSFGateway
			system('sudo mv /usr/local/bin/YSFParrot /usr/local/bin/YSFParrot.old > /dev/null 2>/dev/null &');		// YSFGateway			
			}
          if (escapeshellcmd($_POST['MMDVMModeYSF2DMR']) == 'OFF' ) { 
			$configysf2dmr['Enabled']['Enabled'] = "0";
		    $configdmrgateway['DMR Network 3']['Enabled'] = "0";			
			$configmmdvm['System Fusion Network']['LocalPort'] = "3200";		
			$configmmdvm['System Fusion Network']['GatewayPort'] = "4200";
			
			system('sudo mv /usr/local/bin/YSFGateway.old /usr/local/bin/YSFGateway > /dev/null 2>/dev/null &');		// YSFGateway
			system('sudo mv /usr/local/bin/YSFParrot.old /usr/local/bin/YSFParrot > /dev/null 2>/dev/null &');		// YSFGateway			
			system('sudo systemctl stop ysf2dmr.service > /dev/null 2>/dev/null &');	// YSFGateway
			system('sudo systemctl stop ysf2dmr.timer > /dev/null 2>/dev/null &');		// YSFGateway
			system('sudo systemctl disable ysf2dmr.service > /dev/null 2>/dev/null &');	// YSFGateway
			system('sudo systemctl disable ysf2dmr.timer > /dev/null 2>/dev/null &');		// YSFGateway
			system('sudo systemctl enable ysfgateway.service > /dev/null 2>/dev/null &');		// YSFGateway
			system('sudo systemctl enable ysfgateway.timer > /dev/null 2>/dev/null &');		// YSFGateway
			system('sudo systemctl enable ysfparrot.service > /dev/null 2>/dev/null &');		// YSFGateway
			system('sudo systemctl enable ysfparrot.timer > /dev/null 2>/dev/null &');		// YSFGateway
			system('sudo systemctl start ysfgateway.service > /dev/null 2>/dev/null &');	// YSFGateway
			system('sudo systemctl start ysfgateway.timer > /dev/null 2>/dev/null &');	// YSFGateway
			system('sudo systemctl start ysfparrot.service > /dev/null 2>/dev/null &');	// YSFGateway
			system('sudo systemctl start ysfparrot.timer > /dev/null 2>/dev/null &');		// YSFGateway
			system('sudo mv /usr/local/bin/YSF2DMR /usr/local/bin/YSF2DMR.old > /dev/null 2>/dev/null &');			
	...
	if ( (!isset($configysf2dmr['DMR Network']['TGListFile'])) && (file_exists('/usr/local/etc/TGList.txt')) ) { $configysf2dmr['DMR Network']['TGListFile'] = "/usr/local/etc/TGList.txt"; }
	...
	if ($configysf2dmr['Enabled']['Enabled'] == 0) {
		system('sudo systemctl start ysfgateway.service > /dev/null 2>/dev/null &');		// YSFGateway
...
<?php if (file_exists('/etc/dstar-radio.mmdvmhost')) { ?>
    ...
    <?php
	if ( $configmmdvm['System Fusion']['Enable'] == 1 && $configysf2dmr['Enabled']['Enabled'] == 0) {
    ...
    <tr>
    <td align="left"><a class="tooltip2" href="#">EA7EE YSF2DMR:<span><b>YSF2DMR Mode</b>Turn on YSF2DMR Features</span></a></td>
    <?php
...
<?php
	...
	<div><input type="button" value="<?php echo $lang['apply'];?>" onclick="submitform()" /><br /><br /></div>
<?php } ?>
<?php if (file_exists('/etc/dstar-radio.mmdvmhost') && ($configmmdvm['System Fusion Network']['Enable'] == 1 && $configysf2dmr['Enabled']['Enabled'] == 0) || $configdmr2ysf['Enabled']['Enabled'] == 1 ) {
...
<?php if (file_exists('/etc/dstar-radio.mmdvmhost') && ($configysf2dmr['Enabled']['Enabled'] == 1 )) { ?>
	...
	<input type="hidden" name="confFollowMode" value="OFF" />
	<div><b><?php echo $lang['ysf2dmr_config'];?></b></div>
...
<?php
    ...
    <tr>
      <td align="left"><a class="tooltip2" href="#">YSF2DMR StartUp TG:<span><b>YSF2DMR StartUp TG</b>Enter your DMR StartUp TG here</span></a></td>
	...
	<tr>
    <td align="left"><a class="tooltip2" href="#"><?php echo $lang['aprs_host'];?>:<span><b>APRS Host</b>Set your preferred APRS host here</span></a></td>
    <td colspan="2" style="text-align: left;"><select name="selectedAPRSHost">
<?php
        $testAPSRHost = $configs['aprsHostname'];
    	$aprsHostFile = fopen("/usr/local/etc/APRSHosts.txt", "r");
	...
	<tr>
      <td align="left"><a class="tooltip2" href="#"> <?php echo $lang['aprs_callsign'];?>:<span><b>APRS Callsign</b>Enter Callsign if different for APRS</span></a></td>
      <td align="left" colspan="2"><input type="text" name="ysf2dmrCallsign" size="13" maxlength="9" value="<?php if (isset($configysf2dmr['aprs.fi']['AprsCallsign'])) { echo $configysf2dmr['aprs.fi']['AprsCallsign']; } ?>" /></td>
    </tr>
	
	<tr>
      <td align="left"><a class="tooltip2" href="#"> <?php echo $lang['aprs_beacon_text'];?>:<span><b>APRS Beacon Text</b>Enter text you want on your APRS beacon</span></a></td>
      <td align="left" colspan="2"><input type="text" name="ysf2dmrBeaconText" size="25" maxlength="25" value="<?php if (isset($configysf2dmr['aprs.fi']['Beacon'])) { echo $configysf2dmr['aprs.fi']['Beacon']; } ?>" /></td>
    </tr>

	<tr>
      <td align="left"><a class="tooltip2" href="#"> <?php echo $lang['aprs_beacon_icon'];?>:<span><b>APRS Beacon Icon</b>Enter Icon you want on your APRS beacon</span></a></td>
      <td align="left" colspan="2"><input type="text" name="ysf2dmrBeaconIcon" size="13" maxlength="9" value="<?php if (isset($configysf2dmr['aprs.fi']['Icon'])) { echo $configysf2dmr['aprs.fi']['Icon']; } ?>" /></td>
    </tr>	

	<tr>
      <td align="left"><a class="tooltip2" href="#"> <?php echo $lang['aprs_beacon_time'];?>:<span><b>APRS Beacon Time (min)</b>Enter time period in minutes you want for your APRS beacon</span></a></td>
      <td align="left" colspan="2"><input type="text" name="ysf2dmrBeaconTime" size="13" maxlength="9" value="<?php if (isset($configysf2dmr['aprs.fi']['BeaconTime'])) { echo $configysf2dmr['aprs.fi']['BeaconTime']; } ?>" /></td>
    </tr>
	
    <tr>
    <td align="left"><a class="tooltip2" href="#">Follow Mode:<span><b>Follow Mode</b>HotSpot Icon follow the user by GPS Information.</span></a></td>
    <?php
	if ( isset($configysf2dmr['aprs.fi']['HotSpotFollow']) ) {
		if ( $configysf2dmr['aprs.fi']['HotSpotFollow'] ) {
			echo "<td align=\"left\"><div class=\"switch\"><input id=\"toggle-confFollowMode\" class=\"toggle toggle-round-flat\" type=\"checkbox\" name=\"confFollowMode\" value=\"ON\" checked=\"checked\" /><label for=\"toggle-confFollowMode\"></label></div></td>\n";
		}
		else {
			echo "<td align=\"left\"><div class=\"switch\"><input id=\"toggle-confFollowMode\" class=\"toggle toggle-round-flat\" type=\"checkbox\" name=\"confFollowMode\" value=\"ON\" /><label for=\"toggle-confFollowMode\"></label></div></td>\n";
		}
	} else {
		echo "<td align=\"left\"><div class=\"switch\"><input id=\"toggle-confFollowMode\" class=\"toggle toggle-round-flat\" type=\"checkbox\" name=\"confFollowMode\" value=\"ON\" /><label for=\"toggle-confFollowMode\"></label></div></td>\n";
	}
	...
	<tr>
      <td align="left"><a class="tooltip2" href="#"> <?php echo $lang['aprs_fi_apikey'];?>:<span><b>aprs.fi location server apikey</b>Enter your apikey to get positioning information on your Yaesu Transceiver</span></a></td>
      <td align="left" colspan="2"><input type="text" name="ysf2dmrAprsApikey" size="25" maxlength="25" value="<?php if (isset($configysf2dmr['aprs.fi']['APIKey'])) { echo $configysf2dmr['aprs.fi']['APIKey']; } ?>" /></td>
    </tr>

	<tr>
      <td align="left"><a class="tooltip2" href="#"> <?php echo $lang['voice_beacon_time'];?>:<span><b>Time cadence of voice beacon</b>Enter time for voice beacon (0=OFF)</span></a></td>
      <td align="left" colspan="2"><input type="text" name="ysf2dmrVoiceTime" size="13" maxlength="9" value="<?php if (isset($configysf2dmr['Storage']['BeaconTime'])) { echo $configysf2dmr['Storage']['BeaconTime']; } ?>" /></td>
    </tr>
    </table>
	<div><input type="button" value="<?php echo $lang['apply'];?>" onclick="submitform()" /><br /><br /></div>
<?php } ?>
