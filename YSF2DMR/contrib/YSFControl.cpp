...
bool CYSFControl::writeModem(unsigned char *data, unsigned int len)
{
	...
	case YSF_DT_DATA_FR_MODE:
		LogMessage("Paquete en state:%d con fi:%d,cm:%d,bn:%d,bt:%d,fn:%d,ft:%d,dt:%d,mr:%d.",m_rfState,m_lastFICH.getFI(),m_lastFICH.getCM(),m_lastFICH.getBN(),
				   m_lastFICH.getBT(),m_lastFICH.getFN(),m_lastFICH.getFT(),m_lastFICH.getDT(),m_lastFICH.getMR());	
...
bool CYSFControl::processFRData(bool valid, unsigned char *data)
{
	...
	if (valid && fi == YSF_FI_HEADER) {
		...
		if (m_rfState == RS_RF_LISTENING) {
			// If during listening we get data we rebuild header and put the data out		
